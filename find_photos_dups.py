...
def scan_tree(items_file):
	...
	photostree_root = '/home/silvavlis/tmp/test_fotos'
	files_list_file = '/home/silvavlis/Programming/program_org_fotos/files_list.dat'
	items_list_file = '/home/silvavlis/Programming/program_org_fotos/items_list.dat'
	if os.path.exists(files_list_file) and (os.path.getsize(files_list_file) > 0):
		trace("Valid files list file found, getting files from it")
		fd_tree = open(files_list_file, 'rb')
		media_files= pickle.load(fd_tree)
		fd_tree.close()
	else:
		trace("Valid files list file not found, generating it")
		media_files = TreeScanner(photostree_root)
		fd_tree = open(files_list_file, 'wb')
		pickle.dump(media_files, fd_tree)
		fd_tree.close()
	trace("Tree analyzed: %d files to be processed found." % len(media_files))
	if os.path.exists(items_list_file) and (os.path.getsize(items_list_file) > 0):
		trace("Valid items list file found, getting files from it")
		fd_items = open(items_list_file, 'rb')
		items= pickle.load(fd_items)
		fd_items.close()
	else:
		trace("Valid items list file not found, generating it")
		items = ItemsCollection(media_files)
		fd_items = open(items_list_file, 'wb')
		pickle.dump(items, fd_items)
		fd_items.close()
	trace("%s items result from processing the tree" % len(items.keys()))
	trace("generating the dictionaries")
	items.generate_dicts()
	trace("Tree processed, dumping information")
...
def analyze_tree(items_file):
	...
	items = pickle.load(fd_items)
	fd_items.close()
	trace("Photos information loaded, processing duplicates")
	trace(items.show_dup_cksms())

if (__name__ == '__main__'):
	items_file = '/home/silvavlis/Programming/program_org_fotos/photos_info.dat'
	if not ((os.path.exists(items_file)) and (os.path.getsize(items_file) > 0)):
		trace("No valid photos information file found, scanning tree")
